jQuery(document).ready(function($) {
    $('.profile_field_group_anchor').click(function(e) {
        e.preventDefault();
        $('.profile_field_group_anchor, .profile_field_group').removeClass('active');
        $(this).addClass('active');
        $($(this).attr('href')).addClass('active');
    });
    $(".profile-datepicker").each(function() {
        var $this = $(this);
        $this.datepicker({
    ...
    $('.profile_field_meta_box #field_type').change(function(event) {
        $.ajax({
            type: 'GET',
            url: ajaxurl,
            data: {
                action: 'profile_field_type_options',
                'type': $(this).val(),
                'post_id': $(this).data('post_id')
            },
            success: function(data) {
                $('#profile_field_type_fields').html(data);
            },
            dataType: 'html',
            context: this,
        });
    ...
    $('body').delegate('[data-action="add_repeat_field"]', 'click', function(e) {
        ...
        var count = $('.profile_repeat_field').length - 1;
